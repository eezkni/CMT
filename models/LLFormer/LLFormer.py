...
class LLFormer(nn.Module):
    def __init__(self,
        ...
        self.decoder_level1_0 = nn.Sequential(*[
            TransformerBlock(dim=int(int(dim * 2)), num_heads=heads[1], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[0])])

        self.down_2 = Downsample(int(dim * 2))  ## From Level 1 to Level 2
        self.decoder_level2_0 = nn.Sequential(*[
            TransformerBlock(dim=int(int(dim * 2 * 2)), num_heads=heads[2], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[1])])

        self.down_3 = Downsample(int(dim * 2 * 2))  ## From Level 2 to Level 3
        self.decoder_level3_0 = nn.Sequential(*[
            TransformerBlock(dim=int(int(dim * 2 * 4)), num_heads=heads[3], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[2])])
        self.down_4 = Downsample(int(dim * 2 * 4))  ## From Level 3 to Level 4
        self.decoder_level4 = nn.Sequential(*[
            TransformerBlock(dim=int(int(dim * 2 * 8)), num_heads=heads[3], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[3])])

        self.up4_3 = Upsample(int(dim * 2 * 8))  ## From Level 4 to Level 3
        self.decoder_level3_1 = nn.Sequential(*[
            TransformerBlock(dim=int(int(dim * 2 * 4)), num_heads=heads[3], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[2])])
        ...
        self.decoder_level1_1 = nn.Sequential(*[
            TransformerBlock(dim=int(int(dim * 2)), num_heads=heads[1], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[0])])
    ...
    def forward(self, inp_img):
        # modified in Retinol to make sure that width and height can be divided by 16
        _, _, h, w = inp_img.shape
        pad_h = (h - 1) // 16 * 16 + 16 - h
        pad_w = (w - 1) // 16 * 16 + 16 - w
        if pad_h != 0 or pad_w != 0:
            inp_img = torch.nn.functional.pad(inp_img, pad=(0, pad_w, 0, pad_h), mode='reflect')
        # modify end
    ...
    # modified in Retinol
    def forward_with_tiles(self, lq_img: torch.Tensor, img_size, overlap_pixels=32):
        """
        This function is used for evaluation. It can accept images with any sizes as input.
        Args:
            lq_img: (B, C, H, W) tensor
            overlap_pixels: pixels that overlap between two tiles

        Returns:
            (B, C, H, W) tensor
        """
        b, c, h, w = lq_img.shape

        tile_size = img_size - overlap_pixels
        expand_right = (w // tile_size + 1) * tile_size + overlap_pixels - w
        expand_bottom = (h // tile_size + 1) * tile_size + overlap_pixels - h
        lq_img_padded = torch.nn.functional.pad(lq_img, pad=(0, expand_right, 0, expand_bottom), mode='reflect')
        factor_tensor = torch.zeros((lq_img_padded.shape[2], lq_img_padded.shape[3]), device=lq_img_padded.device)
        result_tensor = torch.zeros_like(lq_img_padded, device=lq_img_padded.device)
        ...
        for h_offset in h_offsets:
            for w_offset in w_offsets:
                tile_img = lq_img_padded[:, :, h_offset: h_offset + img_size, w_offset: w_offset + img_size]
                tile_result = self.forward(tile_img)

                factor_tensor[h_offset: h_offset + img_size, w_offset: w_offset + img_size] += 1
                result_tensor[:, :, h_offset: h_offset + img_size, w_offset: w_offset + img_size] += tile_result

        result_tensor = result_tensor / factor_tensor

        return result_tensor[:, :, 0:h, 0:w]
